...
exports.updateBatch = async (req, res) => {
  try {
      ...
      for (const userId of batch.users) {
        const normalizedProgress = progressUpdates.map((mod) => ({
          ...
          completedLessons: (mod.completedLessons || []).map((lesson) => {
            const userCompletedTopics = (lesson.completedTopics || [])
            ...
            const fallbackTopics = (!lesson.completedTopics || lesson.completedTopics.length === 0)
              ? moduleMap[mod.moduleId.toString()]?.[lesson.lessonId.toString()] || []
              : [];

            const combinedTopics = [...userCompletedTopics, ...fallbackTopics]
              .filter((t, index, self) =>
                t && t.topicId &&
                index === self.findIndex((x) => x.topicId.toString() === t.topicId.toString())
              );

            return {
              lessonId: lesson.lessonId,
              completedTopics: combinedTopics
            };
          })
        }));
    ...
    userProgressDetails.forEach((user) => {
      ...
      // totalCompletedTopics += user.progress.topics.completed;
    });
    ...
    const allProgress = await Progress.find({ user: { $in: users }, course })
      .lean()
      .exec();


    // Generate detailed progress for batch based on user progress
    if (allProgress.length > 0) {
      const detailedMap = new Map();

      allProgress.forEach(progress => {
        progress.detailed?.forEach(module => {
          if (!detailedMap.has(module.moduleId)) {
            detailedMap.set(module.moduleId, new Map());
          }

          const moduleMap = detailedMap.get(module.moduleId);

          module.completedLessons.forEach(lesson => {
            if (!moduleMap.has(lesson.lessonId)) {
              moduleMap.set(lesson.lessonId, new Set());
            }

            lesson.completedTopics.forEach(topicId => {
              if (topicId) {
                moduleMap.get(lesson.lessonId).add(topicId.toString());
              }
            });
          });
        });
      ...
      // Convert the map into desired structure
      for (const [moduleId, lessonsMap] of detailedMap.entries()) {
        const completedLessons = [];

        for (const [lessonId, topicsSet] of lessonsMap.entries()) {
